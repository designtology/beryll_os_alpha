...
// When it starts up in the editor, it will fill the "sceneNames.scenes" array with the names of all your sceneNames.scenes in the build settings
...
// string sceneName = ReadSceneNames.singleton.sceneNames.scenes[2];
...
public class ReadSceneNames : MonoBehaviour
{
	...
	public GlobalVariables sceneNames;
	...
	void LoadLevelNames(){
		...
		if (levelNamesAsset != null){
			...
			if (numLevels > 0){
				sceneNames.scenes = new string[numLevels];
				for (int i = 0; i < numLevels; ++i){
					sceneNames.scenes[i] = br.ReadString();
				}
			}
		}
	}
	...
	private static void UpdateNames(UnityEditor.MenuCommand command)
	{
		...
		context.sceneNames.scenes = ReadNames();
	...
	void SaveNameFile(){
		...
		bw.Write(sceneNames.scenes.Count ());
		for (int i = 0; i < sceneNames.scenes.Count(); ++i){
			bw.Write(sceneNames.scenes[i]);
		}
